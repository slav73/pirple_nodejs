...
handlers.index = function(data, callback) {
  if (data.method === 'get') {
    var templateData = {
      ...
      'head.description':
        'We offer free, simple uptime monitoring for HTTP/HTTPS sites',
      'body.class': 'index',
    }
...
handlers.categoriesCreate = function(data, callback) {
  if (data.method === 'get') {
    var templateData = {
    ...
    helpers.getTemplate('categoriesCreate', templateData, function(err, str) {
      if (!err && str) {
        helpers.addUniversalTemplates(str, templateData, function(err, str) {
          if (!err && str) {
            callback(200, str, 'html')
...
handlers.sessionDeleted = function(data, callback) {
  if (data.method === 'get') {
    var templateData = {
      ...
      'head.description': 'You are logged out from your account',
    ...
    if (trimmedAssetName.length > 0) {
      helpers.getStaticAsset(trimmedAssetName, function(err, data) {
        if (!err && data) {
          ...
          if (trimmedAssetName === '.css') {
            contentType = 'css'
          }
          if (trimmedAssetName === '.png') {
            contentType = 'png'
          }
          if (trimmedAssetName === '.jpg') {
            contentType = 'jpg'
          }
          if (trimmedAssetName === '.ico') {
            contentType = 'favicon'
          }
...
handlers._users.post = function(data, callback) {
  ...
  if (firstName && lastName && phone && password && tosAgreement) {
    ...
    callback(400, { Error: 'User POST: Missing required fields' })
  }
}
  ...
  if (phone) {
    ...
    handlers._tokens.verifyToken(token, phone, function(tokenIsValid) {
      if (tokenIsValid) {
        ...
        callback(403, {
          Error: 'Users GET Missing token in header, or it is invalid',
        })
      }
    })
  } else {
    callback(400, { error: 'Users GET: Missing required fields' })
  }
}
    ...
    if (firstName || lastName || password) {
      ...
      handlers._tokens.verifyToken(token, phone, function(tokenIsValid) {
        if (tokenIsValid) {
          ...
          callback(403, {
            Error: 'Users PUT Missing token in header, or it is invalid',
          })
        }
      })
    } else {
  ...
  if (phone) {
    ...
    handlers._tokens.verifyToken(token, phone, function(tokenIsValid) {
      if (tokenIsValid) {
                    ...
                    _data.delete('checks', checkId, function(err) {
                      ...
                      if (checksDeleted === checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {
                            Error:
                              'Not all the checks were deleted successfully',
                          })
                        }
                      }
                    })
        ...
        callback(403, {
          Error: 'Users DELETE Missing token in header, or it is invalid',
        })
      }
    })
  } else {
    callback(400, { error: 'DELETE: Missing required fields' })
  }
}
...
handlers._tokens.delete = function(data, callback) {
  ...
  if (id) {
    ...
    callback(400, { error: 'Tokens DELETE: Missing required fields' })
  }
}
...
handlers._checks.post = function(data, callback) {
  ...
  var method =
    ...
    ['get', 'post', 'put', 'delete'].indexOf(data.payload.httpmethod) > -1
...
handlers._checks.get = function(data, callback) {
  ...
  if (id) {
    ...
    callback(400, { error: 'Checks GET: Missing required fields' })
  }
}
...
handlers._checks.put = function(data, callback) {
  var id =
    ...
    data.payload.uid.trim().length === 20
    ...
    if (protocol || url || method || successCodes || timeoutSeconds) {
      _data.read('checks', id, function(err, checkData) {
        if (!err && checkData) {
          ...
          callback(400, { Error: 'Check dont exist' })
        }
      })
    } else {
      callback(400, { Error: 'Missing field to update' })
...
handlers._checks.delete = function(data, callback) {
  ...
  if (id) {
    ...
    callback(400, { error: 'Checks DELETE: Missing required fields' })
  }
}
